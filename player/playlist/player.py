...
class PlaylistPlayer:
    ...
    def handle_media_end_reached(self):
        ...
        if not next_index.isValid():
            ...
            self._item = next_index
            self.load_media(self._item)
    ...
    def handle_playlist_end_reached(self):
        ...
        if playback_mode == "off":
            ...
            # self.load_media(self._item, play=True)
    ...
    def skip_previous(self):
        ...
        if not prev_index.isValid():
            ...
            self._item = prev_index.model().item(prev_index.row())
            self.load_media(self._item)
    ...
    def skip_next(self):
        ...
        if not next_index.isValid():
            ...
            self._item = next_index.model().item(next_index.row())
            self.load_media(self._item)
    ...
    def load_media(self, index, play=True):
        if not index.isValid():
            ...
            self._item = index.model().item(index.row())
            mrl = self._item.data(MediaItem.PathRole)
            is_spherical = self._item.data(MediaItem.SphericalRole)
